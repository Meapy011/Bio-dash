...
def hr_callback(data):
    ...
    # 2. Extract R-R Intervals (HRV) smuggled in the HR packet
...
def acc_callback(data):
    """Extracts 200Hz 3-Axis Accelerometer vectors."""
    ...
    for idx, val in enumerate(samples):
        try:
            x = int(getattr(val, 'x', val[0] if isinstance(val, list) else 0))
            y = int(getattr(val, 'y', val[1] if isinstance(val, list) else 0))
            z = int(getattr(val, 'z', val[2] if isinstance(val, list) else 0))
    ...
    while True:
        try:
            ...
            async with PolarDevice(device) as polar:
                ...
                # The crucial fix is right here: adding the '8' for the 8G range!

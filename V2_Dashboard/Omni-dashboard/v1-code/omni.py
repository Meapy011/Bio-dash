...
omni_state = {
    "polar": {"status": "Disconnected", "hr": 0, "rr": 0, "rmssd": 0.0, "sdnn": 0.0, "ecg_mv": 0.0, "acc": [0,0,0]},
...
# --- ADAPTER ROUTING ---
POLAR_ADAPTER = "hci0"    # Dedicated VIP Lane: PCIe Card
GENERAL_ADAPTER = "hci1"  # General Traffic: USB Dongle
...
def polar_ecg_cb(data):
    ...
    if samples:
        try:
            ...
            sys.stdout.write(f"\r[ POLAR ] ❤️ HR: {omni_state['polar']['hr']:3d} | ⏱️ RR: {omni_state['polar']['rr']:4d}ms | ⚡ ECG: {omni_state['polar']['ecg_mv']:>6.3f} mV    ")
            sys.stdout.flush()
        except Exception: pass
...
def viatom_rx_cb(sender, data):
    ...
    try:
        if len(data) >= 9: 
            ...
            # DIAGNOSTIC: Print the raw hex so we can see what the ring is saying
            sys.stdout.write(f"\r[ O2 RAW ] HEX: {raw_hex[:16]}... | SpO2: {spo2_val}% | HR: {hr_val}    ")
            sys.stdout.flush()
...
async def omni_scanner():
    while True:
        ...
        if needs_scan:
            ...
            try:
                # RUN SCANNER EXCLUSIVELY ON USB DONGLE USING NEW BLUEZ KWARG
        ...
        try:
            ...
            if device:
                async with PolarDevice(device) as p:
                    ...
                    # Prevent Polar processor traffic jams with 1.5s delays
                ...
                logging.warning(f"\n⚠️ Target not found on {POLAR_ADAPTER}. Retrying...")
    ...
    while True:
        ...
        try:
            ...
            async with BleakClient(device, timeout=15.0, disconnected_callback=handle_disconnect) as client:
                ...
                # THE FIX: Add "write-without-response" catch
                ...
                logging.info("Viatom telemetry pipeline opened. Sending data requests...")
                
                while omni_state["viatom"]["status"] == "Connected":
                    try:
                        # THE FIX: Fallback to handle stubborn write-without-response firmware
                        try:
                        ...
                        print(f"\n[ O2 WRITE ERROR ]: {write_err}") # Expose silent drops
                    await asyncio.sleep(2)
        except Exception as e: 

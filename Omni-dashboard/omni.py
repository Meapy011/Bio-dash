        ...
        evtSource.onmessage = (e) => {
            ...
            if(data.polar.status === "Connected") {
                ...
                if (rmssd > 0) {
                    ...
                    if (rmssd < 20) {
                        labelEl.innerText = "HIGH STRESS";
                        labelEl.className = "text-[10px] sm:text-xs font-bold mt-2 text-rose-500 text-center w-full h-4";
                    } else if (rmssd < 50) {
                        labelEl.innerText = "BALANCED";
                        labelEl.className = "text-[10px] sm:text-xs font-bold mt-2 text-amber-400 text-center w-full h-4";
                    } else {
                        labelEl.innerText = "RELAXED";
                        labelEl.className = "text-[10px] sm:text-xs font-bold mt-2 text-emerald-400 text-center w-full h-4";
                    }
                }
                ...
                if (data.polar.ecg_buffer && data.polar.ecg_buffer.length > 0) {
                    let startT = now - (data.polar.ecg_buffer.length * 7.69);
                ...
                if (data.polar.acc_buffer && data.polar.acc_buffer.length > 0) {
                    let startT = now - (data.polar.acc_buffer.length * 5.0);
    ...
    while True:
        ...
        try:
            ...
            async with BleakClient(device, timeout=15.0, disconnected_callback=handle_disconnect) as client:
                ...
                try: await client.start_notify(target_notify_uuid, viatom_rx_cb)
                except Exception as e:
                ...
                while omni_state["viatom"]["status"] == "Connected":
                    try:
                        try: await client.write_gatt_char(target_write_uuid, write_bytes, response=True)
                        except Exception: await client.write_gatt_char(target_write_uuid, write_bytes, response=False)
                    except Exception: pass
                    await asyncio.sleep(2)
